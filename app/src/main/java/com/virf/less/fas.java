...
////////////////////////////////////////////////////////////////////////////////////
 ...
 public short[] memory = new short[16000];  //выделяем память из 16 битных целых размером в 2000 
...
*/
  ...
  Handler Hnd;
  public void  setText() {  // int o, int i , int op ,String, s s.length) { obj  num  op  s" ghjh "  
   Message msg;
   int l =  ST.pop();
   int indStr =  ST.pop(); 
...
public class FVM {
  ...
  public void    StringInBuffer(int funk)  {
     switch (funk) {
       case 0 : SB.setLength(0); ports[1] = 0;  // обнуляет строковый буффер
       break;
       case 1 :
               String str=null;  
               if (SB.length()==0) str="";
               ...
               ST.push( appendData(StringVector, str )  ); //appendData возвращает индекс на строку
               ST.push( str.length() );                        // на стеке адрес строки в векторе и длина	        
               SB.setLength(0); 
	       ports[1] = 1;      
  ...
  public void   StWritePort(int prt, int y) { 
     int st = stack.pop();
     if (ports[1] == 0 ) System.out.println("st= "+st ); 
       else SB.append(st+"");
   }    
  ...
  switch(port) {
    case 0 :
      ...
      switch (y) {
        ...
        case 1 :  StWritePort(port,y) ;  break;
...
//      case 6 :   as.     break;               
   ...
   break;
  case 1 : StringInBuffer(y); 
   break; 
  case 2 :
           switch(y) {
	      case 0 :  //  ind text поля   ind string  str length ---  
                   setText();             
            break; 
...
//  public int magicValue = 200; //  если опкод превысит это значение  то или call или jump туда
 ...
 public void processImage( int startIP ) {
  ...
  while  (ip<IMAGE_SIZE  && ! _HALT) {
   ...
   int ind = ST.pop();
   String str =  getStringv(StringVector, ind) ;
   if (ports[1]==0) 
      System.out.println( str );
    else SB.append(str) ;
 }

 /*  // тестирование рефлекшн api 
